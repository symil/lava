...
function generateTypedefs() {
    return getAllTypedefs().map(({baseType, newType}) => {
        ...
        return {
            ...
            definition: [
                `pub type ${getWrappedVkTypeName(newType.name)}${lifetimes} = super::super::${prefix}${getWrappedVkTypeName(baseType.name)}${lifetimes};`,
                `pub type ${getRawVkTypeName(newType.name)} = super::super::${prefix}${getRawVkTypeName(baseType.name)};`
            ]
        };
    });
}
